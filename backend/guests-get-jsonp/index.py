...
def handler(event: dict, context) -> dict:
    """API для получения списка гостей через JSONP"""
    
    method = event.get('httpMethod', 'GET')
    params = event.get('queryStringParameters', {}) or {}
    callback = params.get('callback', 'handleGuestsData')
    ...
    if method != 'GET':
        return {
            ...
            'body': f'{callback}({{\"success\": false, \"error\": \"Method not allowed\"}})',
            'isBase64Encoded': False
        }
    ...
    try:
        ...
        error_response = f'{callback}({{\"success\": false, \"error\": {json.dumps(str(e))}}})'
        return {
            'statusCode': 500,
            'headers': {**cors_headers, 'Content-Type': 'application/javascript'},
            'body': error_response,
